...
- `src/styles/main.css` -- add `[data-theme="dark"]` block overriding `:root` custom properties with dark palette values
- `src/components/board.ts` -- add theme toggle button to the header, wire up click handler to toggle `data-theme` attribute and persist preference
...
- `index.html` -- no structural changes expected; theme applied via `data-theme` attribute on `<html>` or `<body>`
